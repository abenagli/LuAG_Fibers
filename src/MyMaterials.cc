...
//    TF1 * fInduced_vs535 = new TF1 ("fInduced_vs535", "x", 5.6, 2.1);
   	//  	        cout << " ind_abs length = " << ind_abslength << endl;
    for (int i = 0; i < NUMENTRIES_3; i++){
...
//   	  	ABS_LENGTH[i] = (ABS_LENGTH[i]*ind_abslength) / (ABS_LENGTH[i] + ind_abslength);
  ...
  //      TGraph * gAbsorption = new TGraph ();
        for (int i = 0; i < NUMENTRIES_3; i++) {
        	cout << ABS_LENGTH[i] << ", ";
      //  	gAbsorption ->SetPoint (i,ABS_Energy[i], ABS_LENGTH[i] );
        	}
    ...
    double lightyield = 15000 / MeV ;
    mt->AddConstProperty ("SCINTILLATIONYIELD", lightyield );
        cout << " scintillation yield = " << lightyield << endl;
